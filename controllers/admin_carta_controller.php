...
try {
    ...
    switch ($action) {
        case 'createProducto':
            ...
            $okPair = false;
            foreach ($subs as $s) { if ((int)$s['id'] === $p['subcategoria']) { $okPair = true; break; } }
            if (!$okPair) { respondErr('La subcategoría no pertenece a la categoría seleccionada'); }
        ...
        case 'updateProducto':
            $id = (int)($json['id'] ?? 0);
            if ($id <= 0) { respondErr('ID inválido'); }
            $p = [
