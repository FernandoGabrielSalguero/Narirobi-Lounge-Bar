...
<body>
    <div class="layout">
            ...
            <section id="reservas-ui" class="card" aria-labelledby="reservas-filtros-title">
                ...
                <form id="form-filtros" class="form-grid grid-4" role="search" aria-label="Filtros de reservas">
                    ...
                    <div class="form-grid grid-3" style="grid-column: 1 / -1; margin-top: .5rem">
                        <button type="button" class="btn icon-btn" id="btn-aplicar-filtros" aria-label="Aplicar filtros" title="Aplicar">
                            <span class="material-icons" aria-hidden="true">filter_alt</span>
                            <span class="sr-only">Aplicar</span>
                        </button>
                        <button type="button" class="btn icon-btn danger" id="btn-limpiar-filtros" aria-label="Limpiar filtros" title="Limpiar">
                ...
                <!-- Acciones -->
                <button class="btn icon-btn success" id="btn-abrir-crear" aria-controls="modal-crear-reserva" aria-expanded="false" title="Nueva reserva">
                    <span class="material-icons" aria-hidden="true">add_circle</span>
                    <span class="sr-only">Nueva reserva</span>
                </button>
                <button class="btn icon-btn" id="btn-abrir-almanaque" aria-controls="modal-almanaque" aria-expanded="false" title="Almanaque">
                    <span class="material-icons" aria-hidden="true">event</span>
                    <span class="sr-only">Almanaque</span>
                </button>
                <div></div>
        </div>
        ...
        <div id="paginacion" class="form-grid grid-3" style="margin-top:.75rem">
            <button type="button" class="btn icon-btn" id="btn-prev" aria-label="Página anterior" title="Anterior">
                <span class="material-icons" aria-hidden="true">chevron_left</span>
                <span class="sr-only">Anterior</span>
            </button>
            <div id="pagina-info" class="badge info" aria-live="polite">Página 1</div>
            <button type="button" class="btn icon-btn success" id="btn-next" disabled aria-label="Página siguiente" title="Siguiente">
        ...
        <!-- ======= LÓGICA (JS) ======= -->
        <script type="module">
            window.showGlobalSpinner = window.showGlobalSpinner || function() {};
            window.hideGlobalSpinner = window.hideGlobalSpinner || function() {};
            const API = {
            ...
            // Accesibilidad: helpers para abrir/cerrar modales (evita warning de aria-hidden con foco dentro)
            function openModal(id) {
                const m = document.getElementById(id);
                if (!m) return;
                // guardar quién disparó
                const trigger = document.activeElement;
                if (trigger && trigger.id) m.dataset.lastFocusId = trigger.id;

                m.classList.remove('hidden');
                m.setAttribute('aria-hidden', 'false');
                const focusable = m.querySelector('input,select,button,textarea,[tabindex]:not([tabindex="-1"])');
            ...
            function closeModal(id) {
                ...
                // Si el foco está dentro, sacarlo antes de ocultar para no romper lectores de pantalla
                ...
                // Restaurar foco al disparador si existe
                const lastId = m.dataset.lastFocusId;
                if (lastId) {
                    const back = document.getElementById(lastId);
